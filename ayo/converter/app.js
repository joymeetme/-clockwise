...
const addCurrenncyList = document.querySelector(".add-currency-list");
const currenciesList = document.querySelector(".currencies");

let dataURL = "https://api.exchangeratesapi.io/latest";

const initiallyDisplayedCurrencies=["USD", "NGN"];
let baseCurrency;
...
addCurrenncyList.addEventListener("click", addCurrencyListClick);
...
function currenciesListClick(event){
    if(event.target.classList.contains("close")){
        ...
        parentNode.remove();
        addCurrenncyList.querySelector(`[data-currency=${parentNode.id}]`).classList.remove("disabled");
...
function populateAddCurrencyList(){
    for(let i=0; i<currencies.length; i++){
        addCurrenncyList.insertAdjacentHTML(
            "beforeend",
...
function newCurrenciesListItem(currency) {
    ...
    addCurrenncyList.querySelector(`[data-currency=${currency.abbreviation}]`).classList.add("disabled");
